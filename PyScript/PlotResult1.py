...
#td = LoadData(xdmfFilename,'Td0',connect.shape[0],idt=ndt-1,oneDtMem=True,firstElement=-1)
#srd=LoadData(xdmfFilename,'SRd',connect.shape[0],idt=ndt-1,oneDtMem=True,firstElement=-1)
#pn = LoadData(xdmfFilename,'Pn0',connect.shape[0],idt=ndt-1,oneDtMem=True,firstElement=-1)
...
sc = ax0.tripcolor(triang,tmp[0],cmap='plasma',shading='flat',vmin=0.0,vmax=1200.0)
cl = fig.colorbar(sc,ax=ax0)
cl.set_label('temperature (K)')
...
sc = ax1.tripcolor(triang,-pf[0]/1e6,cmap='viridis',shading='flat',vmin=0.0,vmax=20.0)
cl = fig.colorbar(sc,ax=ax1)
cl.set_label('pore pressure (MPa')
