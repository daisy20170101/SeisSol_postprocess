...
srd2= LoadData(xdmfFilename,'SRd',connect.shape[0],idt=int(ndt/4),oneDtMem=True,firstElement=-1)
srd3= LoadData(xdmfFilename,'SRd',connect.shape[0],idt=int(ndt/2),oneDtMem=True,firstElement=-1)
srd4 =LoadData(xdmfFilename,'SRd',connect.shape[0],idt=int(3*ndt/4),oneDtMem=True,firstElement=-1)

#%%

fig,([ax0,ax1],[ax2,ax3])=plt.subplots(nrows=2,ncols=2,figsize=(8,6))

sc = ax0.tripcolor(triang,srd1[0],cmap='plasma',shading='flat',vmin=0,vmax=1)
...
sc = ax1.tripcolor(triang,srd2[0],cmap='plasma',shading='flat',vmin=0,vmax=1)
...
sc = ax2.tripcolor(triang,srd3[0],cmap='plasma',shading='flat',vmin=0.0,vmax=1.0)
...
sc = ax3.tripcolor(triang,srd4[0],cmap='plasma',shading='flat',vmin=0,vmax=1)
